/*
 * 06-27增加dialog传递参数和获取参数的扩展 $.pdialog.open(url, dlgId, title, options, data);
 *06-28 10:25 去掉原本的select组件，恢复原本的数据字典功能
 * */
/*
 * version 1.0.1 增加dialog传递参数和获取参数的扩展 $.pdialog.open(url, dlgId, title, options, data);
 * */
			...
			$.ajax({
				...
				success: function(response) {
					...
					if(typeof response == "object"){
						if(response.resultCode =="10000002"){
							window.location = "" ;
						}
					}
		...
		theme: function(options) {
			...
			return this.each(function() {
				...
				var setTheme = function(themeName) {
					$("head").find("link[href$='style.css']").attr("href", 'css/themes/' + themeName + '/style.css');
...
(function($) {
	var currentDialogId = ""; //获取当前dialog传过来的dat
	var dialogData; //传递过来的参数
	var dialogCbData ;//dialog关闭时返回的参数
	$.pdialog = {
		_op: {
			height: 360,
			width: 800,
		...
		getCurrendDialogId: function() {
			return currentDialogId;
		},
		getDialogOpenData: function() {
			return dialogData;
		},
		...
		closeCurrent: function(cbData) {
			dialogCbData = cbData || "" ;
			this.close($.pdialog._current);
		},
		getDialogCbData : function () {
			return dialogCbData ;
		},
		checkTimeout: function() {
